...
const mixedQuestions = questions.concat(mediumQuestions, hardQuestions);
const startButton = document.getElementById("start");

document
  .querySelector("#numberOfQuestions")
  .addEventListener("input", () => (startButton.disabled = false));

document.querySelector("#numberOfQuestions").addEventListener("change", (e) => {
  numberOfQuestions = e.target.value;
  if (e.target.value != "") {
    if (Number(e.target.value) > 30) {
      document.querySelector("#numberOfQuestions").value = 30;
      numberOfQuestions = 30;
    }
    if (Number(e.target.value) < 10) {
      document.querySelector("#numberOfQuestions").value = 10;
...
startButton.addEventListener("click", () => {
  ...
  switch (divSelectedDifficulty?.innerText || "MIXED") {
    case "EASY":
      difficultyQuestion = questions;
      break;
    case "MEDIUM":
      difficultyQuestion = mediumQuestions;
      break;
    case "HARD":
      difficultyQuestion = hardQuestions;
      break;
    case "MIXED":
      difficultyQuestion = mixedQuestions.slice(0, Number(numberOfQuestions));
      document.querySelector("#totalQuestions").innerText =
        " /" + numberOfQuestions;
  }
...
const addQuestion = (i, difficultyQuestion) => {
  // Aggiungo la domanda
...
const answersCalculator = (i, difficultyQuestion) => {
  // Faccio un array di sole risposte
  const allAnswer = [];
  allAnswer.push(difficultyQuestion[i].correct_answer);
  difficultyQuestion[i].incorrect_answers.forEach((answer) =>
    allAnswer.push(answer)
  );
  return allAnswer;
};

const addCheckedClass = () => {
  // Aggiungo checked class quando cliccato
  const allAnswerDiv = document.querySelectorAll(".answerDiv");
  allAnswerDiv.forEach((el) =>
    el.addEventListener("click", () => {
      allAnswerDiv.forEach((div) => div.classList.remove("checked"));
      el.classList.add("checked");
      if (el.innerText != "MIXED") {
        startButton.disabled = false;
        document.querySelector("#numberOfQuestions").disabled = true;
      } else {
        document.querySelector("#numberOfQuestions").disabled = false;
      }
    })
  );
};
...
const addAnswer = (difficultyQuestion) => {
  // Aggiungo le risposte
...
const addQuestionNumber = () => {
  // Aggiungo counter delle domande
  const questionNumber = document.getElementById("questionNumber");
  questionNumber.innerText = questionCounter;
  questionCounter += 1;
};

const addAnswerVerification = (difficultyQuestion) => {
  // Verifico se la risposta è corretta ed incremento se affermativo
...
const goToNextQuestion = (difficultyQuestion) => {
  // Aggiungo evento al click del bottone che lancia le funzioni precedenti o
